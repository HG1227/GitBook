...
### 3.Nginx VS Apache {#3nginx-vs-apache}
...
Nginx具有很高的稳定性；支持热部署；代码质量非常高，代码很规范，手法成熟，模块扩展也很容易；采用了一些os提供的最新特性如对sendfile\(Linux2.2+\),accept-filter（FreeBSD4.1+）,TCP\_DEFER\_ACCEPT\(Linux 2.4+\)的支持，从而大大提高了性能。
...
### 1.master进程和worker进程的作用 {#1master进程和worker进程的作用}
...
#### Master进程接收到信号是怎样进行处理的？ {#master进程接收到信号是怎样进行处理的？}
...
#### Worker进程接收到信号是怎样进行处理的？ {#worker进程接收到信号是怎样进行处理的？}
...
#### ③管理进程负责监控工作进程的状态，并负责管理其行为 {#③管理进程负责监控工作进程的状态，并负责管理其行为}
...
### 1.Nginx模块常规的HTTP请求和响应的过程： {#1nginx模块常规的http请求和响应的过程：}
...
### 2.Nginx作为Http反向代理服务器的用法： {#2nginx作为http反向代理服务器的用法：}
...
Nginx作为HTTP服务器以及反向代理服务器：![](https://hxgqh.gitbooks.io/haomotraining/content/nginx/nginx421.png)Nginx作为反向代理服务器时转发请求的流程：
